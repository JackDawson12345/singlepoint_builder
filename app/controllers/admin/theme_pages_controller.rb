class Admin::ThemePagesController < Admin::BaseController
  ...
  def inner_add_component
    ...
    # Get the maximum position from existing components and add 1
    max_position = @themePage['inner_pages_components'].map { |comp| comp[:position] || comp['position'] || 0 }.max || 0
    new_position = max_position + 1

    @themePage['inner_pages_components'] << {component_id: params[:component_id].to_i, component_page_id: SecureRandom.uuid, position: new_position}
    @theme.save
    ...
    respond_to do |format|
      ...
      format.turbo_stream do
        ...
        @components = Component.all
        components_count = @components.count
        page_components_count = @themePage['inner_pages_components'].count

        # Use map and join instead of string concatenation
        if @themePage['inner_pages_components'].any?
          sorted_components = @themePage['inner_pages_components'].sort_by { |component| component[:position] || component['position'] || 0 }
          page_components_html = sorted_components.map do |inner_theme_page_component|
            render_to_string(
              partial: 'admin/theme_pages/inner_theme_page_component',
              locals: { theme_page_component: inner_theme_page_component },
              formats: [:html]
            )
          end.join
        else
        ...
        # Use map and join for available components too
        available_components_html = @components.map do |inner_component|
          render_to_string(
            partial: 'admin/theme_pages/inner_component',
            locals: { component: inner_component },
            formats: [:html]
          )
        end.join
  ...
  def inner_remove_component
    ...
    # Remove the component from the components array
    @themePage['inner_pages_components'].reject! do |comp|
      (comp['component_id'] || comp[:component_id]).to_s == component_id_to_remove
    end
    ...
    respond_to do |format|
      ...
      format.turbo_stream do
        ...
        if @themePage['inner_pages_components'].any?
          @themePage['inner_pages_components'].sort_by { |component| component[:position] || component['position'] || 0 }.each do |inner_theme_page_component|
            page_components_html += render_to_string(
              partial: 'admin/theme_pages/inner_theme_page_component',
              locals: { theme_page_component: inner_theme_page_component },
              formats: [:html]
        ...
        # Build the available components HTML
        available_components_html = '<div class="space-y-3">'
        @components.each do |inner_component|
          available_components_html += render_to_string(
            partial: 'admin/theme_pages/inner_component',
            locals: { component: inner_component },
            formats: [:html]
  ...
  def inner_reorder_components
    ...
    positions.each do |pos_data|
      ...
      # Find and update the component position
      @themePage['inner_pages_components'].each do |comp|
        if (comp['component_id'] || comp[:component_id]).to_s == component_id
          # Only update one key, remove the duplicate assignment
          if comp.key?('component_id')
            comp['position'] = new_position
